...
export function useParticlesCanvas(): void {
  useEffect(() => {
    ...
    const start = () => {
      ...
      const targetInterval = isMobile ? 1000 / 30 : 1000 / 45; // 30fps mobile, 45fps desktop
      let lastFrame = 0;
      ...
      const update = (now: number) => {
        ...
        // Grid lines — reduced density on mobile
        ...
        // Ambient orbs — skip on mobile for performance
        const orbs = isMobile
          ? [
    ...
    // Defer canvas initialization to not block initial render
